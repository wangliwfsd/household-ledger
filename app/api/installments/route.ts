...
async function syncLedger(sql: ReturnType<typeof db>) {
  const rows =
    await sql`select principal_per_period::float as principal,fee_per_period::float as fee,paid_periods as paid,total_periods as total,first_payment_date::text as first_date from installments`;
  const now = new Date();
  const balance = rows.reduce((sum, row) => {
    const first = new Date(String(row.first_date) + "T00:00:00"),
      ...
      remaining = Math.max(
        0,
        Number(row.total) - Math.max(Number(row.paid), automatic),
      );
...
export async function GET() {
  try {
    const sql = db();
    const installments =
      await sql`select id,platform,item,total_principal::float as "totalPrincipal",principal_per_period::float as "principalPerPeriod",fee_per_period::float as "feePerPeriod",paid_periods as "paidPeriods",total_periods as "totalPeriods",first_payment_date::text as "firstPaymentDate" from installments order by id`;
    const mortgage =
      (
    ...
    if (b.import) {
      await sql.begin(async (tx) => {
        ...
        for (const x of b.installments)
          await tx`insert into installments(platform,item,total_principal,principal_per_period,fee_per_period,paid_periods,total_periods,first_payment_date) values(${x.platform},${x.item || null},${x.totalPrincipal},${x.principalPerPeriod},${x.feePerPeriod},${x.paidPeriods},${x.totalPeriods},${x.firstPaymentDate})`;
        const m = b.mortgage;
      ...
      await sql`insert into installments(platform,item,total_principal,principal_per_period,fee_per_period,paid_periods,total_periods,first_payment_date) values(${b.platform},${b.item || null},${b.totalPrincipal},${b.principalPerPeriod},${b.feePerPeriod || 0},${b.paidPeriods || 0},${b.totalPeriods},${b.firstPaymentDate})`;
...
export async function PATCH(request: Request) {
  try {
    ...
    if (b.mortgage) {
      ...
      await sql`update installments set platform=${x.platform},item=${x.item || null},total_principal=${x.totalPrincipal},principal_per_period=${x.principalPerPeriod},fee_per_period=${x.feePerPeriod},paid_periods=${x.paidPeriods},total_periods=${x.totalPeriods},first_payment_date=${x.firstPaymentDate} where id=${x.id}`;
      await syncLedger(sql);
    } else {
